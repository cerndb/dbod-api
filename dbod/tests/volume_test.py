...
class ClusterTest(AsyncHTTPTestCase):
    ...
    def test_get_volume_by_instance_name(self):
        ...
        response = self.fetch("/api/v1/volume/node01")
    ...
    def test_create_delete_volume(self):
        ...
        cluster = """{"instance_id": 7, "volume_type_id": "2", "server": "NAS-server", "mounting_path": "/MNT/data01", "attributes": [{ "test": "testvalue"}, {"test01" : "test01value" }], "file_mode": "0755", "owner": "TSM", "group": "ownergroup", "mount_options": "rw,bg,hard"}"""
        print "test_create_delete_volume"

        # Create the instance
        response = self.fetch("/api/v1/volume/create", method='POST', headers={'Authorization': self.authentication}, body=cluster)
        ...
        response = self.fetch("/api/v1/volume/" + str(data[2]["id"]) , method='DELETE', headers={'Authorization': self.authentication})
        self.assertEquals(response.code, 201)
